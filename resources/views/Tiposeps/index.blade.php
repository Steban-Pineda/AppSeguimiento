...
@section('content_header')
    <h1 style="font-weight:800; color:#00324D;">
        <i class="fas fa-hospital mr-2" style="color:#39A900;"></i>
        Gestión de Tipos de EPS
    </h1>
@stop
...
@section('content')
    <style>
        ...
        .table-eps { border-collapse: separate; border-spacing: 0 6px; width: 100%; }
        ...
        .table-eps tbody tr:hover {
            transform: translateY(-2px);
            box-shadow: 0 6px 20px rgba(0,50,77,0.1);
        }

        .table-eps tbody tr td { border: none; padding: 14px 16px; vertical-align: middle; font-size: 0.88rem; }
        ...
        .denominacion-text { font-weight: 600; color: var(--sena-blue); font-size: 0.9rem; }
        .obs-text { color: #6c757d; font-size: 0.82rem; }
    ...
    @if(session('success'))
        <div class="alert alert-success alert-dismissible fade show shadow-sm mb-4" style="border-radius:12px; border-left:5px solid #39A900;">
            <i class="fas fa-check-circle mr-2"></i> {{ session('success') }}
            <button type="button" class="close" data-dismiss="alert"><span>&times;</span></button>
        </div>
    @endif

    <div class="card card-eps">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="mb-0" style="color:white; font-weight:700; display:flex; align-items:center; gap:10px;">
                    <i class="fas fa-hospital-user" style="color:#39A900;"></i>
                    Registros de EPS
                </h5>
                <a href="{{ route('tiposeps.create') }}" class="btn btn-registrar">
                    <i class="fas fa-plus-circle mr-2"></i> Agregar Nueva EPS
                </a>
            </div>
        </div>
        ...
        <div class="card-body p-4">
            ...
            <div class="search-wrapper">
                <form action="{{ route('tiposeps.index') }}" method="GET">
                    <div class="d-flex align-items-center">
                        <div class="input-group flex-grow-1">
                            <input type="text" name="buscar" class="form-control"
                                   placeholder="🔍  Buscar por Nombre, NIS o Documento..."
                        ...
                        @if(request('buscar'))
                            <a href="{{ route('tiposeps.index') }}" class="btn btn-limpiar ml-2">
            ...
            <div class="table-responsive">
                <table class="table table-eps">
                    ...
                    <tr>
                        ...
                        <th style="width: 120px;">Núm. Doc</th>
                        <th>Denominación</th>
                        <th>Observaciones</th>
                    ...
                    @forelse($tiposeps as $item)
                        <tr>
                            ...
                            <td><span class="numdoc-badge">{{ $item->numdoc }}</span></td>
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center" style="gap:6px;">
                                    <a href="{{ route('tiposeps.show', $item->NIS) }}" class="btn-ver" title="Ver detalle">
                                        <i class="fas fa-eye fa-sm"></i>
                                    </a>
                                    <a href="{{ route('tiposeps.edit', $item->NIS) }}" class="btn-editar" title="Editar">
                                    ...
                                    <form action="{{ route('tiposeps.destroy', $item->NIS) }}" method="POST" style="display:inline">
                                        ...
                                        <button type="submit" class="btn-borrar" title="Eliminar"
                                                onclick="return confirm('¿Eliminar esta EPS?')">
                        ...
                        <tr>
                            <td colspan="5">
                                <div class="text-center py-5 text-muted">
                                    ...
                                    <p>No se encontraron registros de EPS.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <div class="footer-info">
            <span class="text-muted small">
                ...
                Mostrando <b>{{ $tiposeps->firstItem() }}</b> al <b>{{ $tiposeps->lastItem() }}</b> de <b>{{ $tiposeps->total() }}</b> registros
            </span>
            <div>{{ $tiposeps->appends(request()->query())->links() }}</div>
        </div>
    </div>
@stop
